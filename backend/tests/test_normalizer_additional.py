...
def test_malformed_kv_values():
    raw = {
        "message": 'devname=H msg="x" srcip=1.1.1.1 dstip=2.2.2.2 srcport=abc dstport=NaN severity=MEDIUM'
    }
    out = normalize(raw)
...
def test_ports_and_attackid():
    raw = {
        "message": 'devname=Host msg="attack event" srcip=1.2.3.4 dstip=5.6.7.8 srcport=443 dstport=1111 attackid=285212772 attack=udp_flood count=42 proto=udp'
    }
    out = normalize(raw)
